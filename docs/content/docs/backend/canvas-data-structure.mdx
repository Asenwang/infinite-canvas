---
title: 画布数据结构
description: 画布运行时数据、用户文档结构、媒体文件与清理机制
---

# 画布数据结构

本文档说明当前画布运行时数据结构、媒体文件的处理方式，以及登录后写入 PostgreSQL 的用户文档边界。

## 当前存储位置

当前运行时会使用腾讯云 COS 作为媒体文件存储，数据库里只保存文件标识和元信息：

- 画布项目 JSON：同步到 PostgreSQL `canvas.user_documents.domain = canvas`；前端运行时仍会镜像到 `localForage` 的 `infinite-canvas:canvas_store`。
- 我的素材 JSON：同步到 PostgreSQL `canvas.user_documents.domain = assets`；前端运行时仍会镜像到 `localForage` 的 `infinite-canvas:asset_store`。
- 生图记录 JSON：同步到 PostgreSQL `canvas.user_documents.domain = image-workbench`。
- 图片、视频、音频文件：上传到腾讯云 COS `creative-drawer/` 前缀。

写入 PostgreSQL 前，图片节点、视频节点、助手图片和素材媒体会优先上传到 COS，再只把 `storageKey` 写入数据库，避免数据库里保存大体积 base64 或失效的 `blob:` URL。
